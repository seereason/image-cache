...
import Data.FileCache.ImageRect ( HasImageRect(imageRect), ImageRect, makeImageRect, scaleImageRect, scaleFromDPI, cropImageRect, uprightImageRect, imageAspect )
...
-- the images.
screenDPI :: Rational
screenDPI = 100.0 :: Rational
printerDPI :: Rational
printerDPI = 600.0 :: Rational

-- | Return the ImageSize value for the image enlarged to 8x10 size
-- (per IRS requirements.)  If the image is wider than it is high it
-- will be rotated 90 degrees ccw by LaTeX, so we need to return
-- "width 10" rather than "height 10" or "width 8".
enlargedSize :: ImageRect -> ImageSize
enlargedSize i =

    -- Examples:
    --   landscape shaped image which is width limited needs the argument width=10in
    --   landscape image which is not width limited needs height=8in
    --   portrait image which is not width limited needs height=10in
    --   portrait image which is width limited needs width=8in
    ImageSize {_units = Inches,
               _size = case squareish i of
                        False -> irsHeight
                        True -> irsWidth,
               _dim = case portrait i of
                       True -> case squareish i of
                                 False -> TheHeight
                                 True -> TheWidth
                       False -> case squareish i of
                                  False -> TheWidth
                                  True -> TheHeight}

portrait :: ImageRect -> Bool
portrait i = imageAspect i >= 1

squareish :: ImageRect -> Bool
squareish i = 1 / pageAspect <= imageAspect i && imageAspect i <= pageAspect

pageAspect :: Rational
pageAspect = irsHeight / irsWidth

-- | Size of the image enlargements required by the internal revenue
-- service.
irsHeight, irsWidth :: Rational
irsHeight = 10.0
irsWidth = 8.0

-- | Determines the screen size of a branding image
brandingKey :: ImageKey -> ImageKey
brandingKey i = scaledKey (ImageSize {_dim = TheHeight, _size = 1.8, _units = Cm}) screenDPI i

-- | Determines the screen size of a signature image
signatureKey :: ScaledKey ImageSize a => a -> ImageKey
signatureKey = scaledKey (ImageSize TheHeight 0.5 Inches) screenDPI

-- | This size is what I measured on the current site
thumbKey :: ScaledKey ImageSize a => a -> ImageKey
thumbKey = scaledKey (ImageSize {_dim = TheHeight, _size = 1.0, _units = Inches}) screenDPI

-- | key to retrieve an image that will be about 3x5 on the screen.
screenKey :: ScaledKey ImageSize a => a -> ImageKey
screenKey = scaledKey (ImageSize {_dim = TheArea, _size = 15.0, _units = Inches}) screenDPI

-- | Get size and all other scaling and editing info from a
printedKey :: (ScaledKey ImageSize a, HasImageSize a) => a -> ImageKey
printedKey img = scaledKey (imageSize img) printerDPI img

enlargedKey :: (ScaledKey ImageSize a, HasImageRect a) => a -> Maybe ImageKey
enlargedKey img =
  case imageRect img of
    Left _ -> Nothing
    Right rect -> Just $ enlargedKey' img rect

enlargedKey' :: ScaledKey ImageSize a => a -> ImageRect -> ImageKey
enlargedKey' img rect =
  scaledKey (enlargedSize rect) printerDPI img
