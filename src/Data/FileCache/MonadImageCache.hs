...
uprightImage orig = do
  ...
  bs' <- liftIO $ $logException ERROR $ normalizeOrientationCode (P.fromStrict bs)
  either
    (\_ -> return (Right orig))
    (\bs'' -> fileFromBytes (liftIO . $logException ERROR . getFileType) fileExtension (P.toStrict bs'') >>= makeImageFile)
    bs'
