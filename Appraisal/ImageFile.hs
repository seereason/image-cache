...
makeImageFile :: forall m. MonadFileCache m => File -> m ImageFile
...
-- @file -b@.
getFileType :: MonadFileCache m => FilePath -> m ImageType
getFileType path =
...
scaleImage :: forall m f. (MonadFileCache m, RealFloat f) => f -> ImageFile -> m ImageFile
